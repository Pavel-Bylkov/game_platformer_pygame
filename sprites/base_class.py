...
class BaseSprite(pg.sprite.Sprite):
    ...
    area - "место обитания" - готовый прямоугольник (pygame rect), внутри которого
    спрайт должен оставаться
    """
    def __init__(self, image, x, y, area, x_speed=0, y_speed=0):
        # Вызываем конструктор класса (Sprite):
        super().__init__()

        # каждый спрайт должен хранить свойство image - картинка.
        self.image = image
        # каждый спрайт должен хранить свойство rect - прямоугольник.
        # Это свойство нужно для определения касаний спрайтов и удобного вывода на экран.
        ...
        # для тех, кто смещается вместе с этим спрайтом, нужно знать, насколько спрайту
        # удалось переместиться за последний update:
        self.x_changed = 0
        self.y_changed = 0
        # направление: вправо = 1, влево = -1 :
        self.direction = -1 if self.x_speed < 0 else 1
    ...
    def update(self, game):
        ...
        проверяет, не надо ли подкорректировать и развернуть спрайт
...
class Character(BaseSprite):
    """Игровые персонажи"""
    def __init__(self, image, x, y, area, x_speed=0, y_speed=0,
                 die_x=False, die_y=True, heavy=GRAVITY):  # TODO добавить позже graph_index=gr_hero
        ...
        self.die_y = die_y
        self.heavy = heavy
        # TODO добавить позже self.graph_index = graph_index
    ...
    def check_dir(self, game):
        ...
        #     else:
        #         self.image = game.costumes[self.graph_index + gr_total]
        #         вторая копия списка содержит зеркальные копии
    ...
    def update(self, game):
        ...
        """
        # 1. Для совместимости с базовым классом тоже будем подсчитывать
        # x_changed и y_changed:
        old_x = self.rect.x
        old_y = self.rect.y

        # 2. Покатаем спрайт со всеми платформами, которые на него успели заехать
        # до его изменений, в том числе и той, на которой он стоит:
        #   2A. Применяем изменения координат платформ, которых коснулись.
        platforms_touched = pg.sprite.spritecollide(self, game.barriers, False)
        ...
        if self.stands_on:
            # оттолкнулись от платформы и ушли на y_speed относительно неё
            if (self.y_speed < 0
        ...
        if self.y_speed > 0:  # идем вниз
            for p in platforms_touched:
                if self.heavy > 0:
                    # при касании с препятствием по y вертикальная скорость пропадает:
                    self.y_speed = 0

                # персонаж, который управляется не гравитацией, а "ареалом", продолжает
                # попытки пройти проверяем, какая из платформ снизу самая высокая,
